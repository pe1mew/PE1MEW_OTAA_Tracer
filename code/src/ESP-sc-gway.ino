...
// Contributions for tracer functions
#include "PE1MEW_packetTracer.h"  /// Tracer functions
#include "PE1MEW_Button.h"        /// Functions to control the program
#include "PE1MEW_Led.h"           /// Functions to control the buzzer
...
/// Tracer object for tracing functions
packetTracer tracer = packetTracer(&display);

/// Button object for input to tracer
const int buttonPin = 2;     /// the number of the pushbutton pin (D4)
PE1MEW_Button button = PE1MEW_Button(buttonPin);
eStateButton buttonState;

/// buzzer (LED object) for signaling from tracer function
const int buzzerPin = 0;     /// the number of the pushbutton pin (D3)
PE1MEW_Led buzzer = PE1MEW_Led(buzzerPin);

/*---------- End of additions by PE1MEW ---------- */
...
void pullData() {
    ...
    pullDataReq[7]  = 0xAF;                   // Modification for RFSEE to make mac address more unique
    pullDataReq[8]  = 0x5E;                   // Modification for RFSEE to make mac address more unique
...
void sendstat() {
    ...
    status_report[7]  = 0xAF;
    status_report[8]  = 0x5E;
    ...
    // int j = snprintf((char *)(status_report + stat_index), STATUS_SIZE-stat_index,
		...
		/// pe1mew: Removed latitude, longitude, and altitude from status report.
    int j = snprintf((char *)(status_report + stat_index),
                              STATUS_SIZE-stat_index,
		                          "{\"stat\":{\"time\":\"%s\",\"rxnb\":%u,\"rxok\":%u,\"rxfw\":%u,\"ackr\":%u.0,\"dwnb\":%u,\"txnb\":%u,\"pfrm\":\"%s\",\"mail\":\"%s\",\"desc\":\"%s\"}}",
		                          stat_timestamp,
                              cp_nb_rx_rcv,
                              cp_nb_rx_ok,
                              cp_up_pkt_fwd,
...
/*------------- Additions by PE1MEW -------------- */
  ...
  /// Handeling for input (button) and output (buzzer) for tracer functions
  buzzer.process();
  buttonState = button.process();
  switch(buttonState)
  {
    case STATE_PRESSED:       // Button is pressed, show info on display
      tracer.showSelectedDevice();
      break;

    case STATE_PRESSED_T1:   // Button is pressend long, switch to next device.
      tracer.changeSelectedDevice();
      buzzer.startBlink(2, 100);
      break;
  ...
  {							//
    ...
    {
#if DUSB>=1
			if (debug>=1) Serial.println(F("MSG:: _state set to S_SCAN"));
#endif
			_state = S_SCAN;
			hop();
			cadScanner();
